...
const loadFormData = () => {
    try {
        ...
        formData.educations.forEach(edu => {
            ...
            eduCounter = Math.max(eduCounter, parseInt(exp.id.split('-')[1]) + 1);
        });
...
// Saytdan chiqishda localStorage ni tozalash
...
const downloadCv = async (format) => {
    ...
    try {
        ...
        cvPreview.style.height = 'auto'; // Balandlikni avtomatik qilish

        const canvas = await window.html2canvas(cvPreview, {
            scale: 2,
            useCORS: true,
            backgroundColor: customization.bgColor,
            height: cvPreview.scrollHeight // Haqiqiy kontent balandligini olish
        });
        ...
        if (format === 'pdf') {
            ...
            // Agar kontent bir sahifaga sig'sa
            if (imgHeight <= pdfHeight) {
                pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, imgHeight);
            } else {
                // Ko'p sahifali kontent uchun
                let position = 0;
        ...
        pdfLoader.style.display = 'none';
        cvPreview.style.height = ''; // Balandlikni qayta tiklash
    }
};
